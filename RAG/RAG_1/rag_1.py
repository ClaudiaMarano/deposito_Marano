...
def validate_env():
    required = ["AZURE_OPENAI_API_KEY", "AZURE_OPENAI_ENDPOINT", "AZURE_MODEL", "AZURE_EMBEDDING_MODEL"]
    missing = [v for v in required if not os.getenv(v)]
    if missing:
        raise RuntimeError(f"Variabili d'ambiente mancanti: {', '.join(missing)}. Controlla il file .env (nessuna riga di commento con '//', usa '#' e nessuno spazio intorno a '=').")

    # Controllo rapido dell'endpoint: rileva placeholder comuni e verifica la risoluzione DNS
    endpoint = os.getenv('AZURE_OPENAI_ENDPOINT', '')
    if not endpoint or 'your-endpoint' in endpoint.lower():
        raise RuntimeError(
            "Valore AZURE_OPENAI_ENDPOINT non valido: sostituisci 'https://your-endpoint.cognitiveservices.azure.com/' con l'endpoint reale del tuo servizio Azure."
        )

    parsed = urlparse(endpoint)
    host = parsed.hostname
    if not host:
        raise RuntimeError(f"AZURE_OPENAI_ENDPOINT non è un URL valido: {endpoint}")

    try:
        socket.getaddrinfo(host, None)
    except socket.gaierror:
        raise RuntimeError(
            f"Impossibile risolvere il nome host '{host}'. Controlla la connettività di rete e il valore di AZURE_OPENAI_ENDPOINT.")
